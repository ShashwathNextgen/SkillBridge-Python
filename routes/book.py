...
def home(id):
	...
	if id != None:
		...
		if user_manager.user.isLoggedIn():
			user_books = book_manager.getReservedBooksByUser(user_id=user_manager.user.uid())['user_books'].split(',')
		...
		user_books=[]
		if user_manager.user.isLoggedIn():
			reserved_books = book_manager.getReservedBooksByUser(user_id=user_manager.user.uid())
